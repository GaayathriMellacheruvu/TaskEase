...
async def chat_with_gpt3_turbo(username: str, collection_name: str, user_input: str = Form(...)):
    try:
        ...
        # Send user input to the Flask proxy server
        proxy_server_url = "http://127.0.0.1:5000/gpt3-turbo/"
        response = requests.post(
            proxy_server_url,
            json={"user_input": user_input}
        )

        # Extract the response from the proxy server
        data = response.json()
        gpt3_turbo_response = data["message"]
